...
Quantum computing promises to address specific problems that are otherwise unsolvable. Here I show how turn a problem into a quantum algorithm, run noiseless and noisy numerical simulations on classical hardware, and send subroutines to a quantum computers. $\langle A \rangle = \text{Tr}(\rho \sum_i a_i P_i) = \sum_i a_i \text{Tr}(\rho P_i)$ with $\rho = \sum_n p_n |\psi_n\rangle\langle\psi_n|$
...
This publication list from [Google Scholar](https://scholar.google.com/citations?hl=en&user=fddyK-wAAAAJ) includes Google Quantum AI research milestone papers as a team collaborator, as well as focus papers with direct contributions as co-author †
